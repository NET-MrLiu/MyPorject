...
namespace Nisus.Trade.Authorization.Users
{
    ...
    /// </summary>
    public class UserEmailer : TradeServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        public async Task sendCodeEmail(string EmailAddress,string code)
        {
            var emailTemplate = GetTitleAndSubTitle(1, L("EmailActivation_Title"), L("EmailActivation_SubTitle"));
            var mailMessage = new StringBuilder();

            mailMessage.AppendLine("<b>" + L("You code") + "</b>: " + code +  "<br />");
            await ReplaceBodyAndSend(EmailAddress, L("EmailActivation_Subject"), emailTemplate, mailMessage);
        }
